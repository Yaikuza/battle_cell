...
func _ready() -> void:
	...
	dna_label.text = "DNA: %d  Balance: %d" % [MetaManager.dna_total, MetaManager.dna_balance]
	dna_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	dna_label.position = Vector2(0, vp.y * 0.03 + 36)
	dna_label.size = Vector2(vp.x, 20)
	dna_label.add_theme_font_size_override("font_size", 13)
	...
	if tid_name:
		...
		tid_label.position = Vector2(0, vp.y * 0.03 + 56)
		tid_label.size = Vector2(vp.x, 18)
		tid_label.add_theme_font_size_override("font_size", 11)
	...
	for i in TAB_LABELS.size():
		...
		btn.position = Vector2(i * tw, vp.y * 0.12)
		btn.size = Vector2(tw - 4, 30)
	...
	var back = Button.new()
	back.text = "  Back"
	back.position = Vector2(vp.x / 2 - 60, vp.y * 0.92)
...
func _update_dna() -> void:
	...
	if dl:
		dl.text = "DNA: %d  Balance: %d" % [MetaManager.dna_total, MetaManager.dna_balance]
...
func _make_card(parent: Node, up_id: String, name: String, desc: String, lv: int, max_lv: int, cost: int, y: float, vp: Vector2, on_buy: Callable) -> void:
	...
	var dc = Label.new()
	dc.text = desc
	dc.position = Vector2(vp.x * 0.07, y + 26)
	dc.size = Vector2(vp.x * 0.55, 20)
	dc.add_theme_font_size_override("font_size", 10)
	...
	if lv < max_lv:
		var buy = Button.new()
		buy.text = "%d DNA" % cost
		buy.position = Vector2(vp.x * 0.78, y + 10)
		buy.size = Vector2(vp.x * 0.17, 32)
		...
		var max_l = Label.new()
		max_l.text = "MAX"
		max_l.position = Vector2(vp.x * 0.78, y + 12)
		max_l.size = Vector2(vp.x * 0.17, 28)
...
func _show_legacy() -> void:
	...
	for form_id in MetaManager.LEGACY:
		...
		if unlocked:
			var ul = Label.new()
			ul.text = "UNLOCKED"
			ul.position = Vector2(vp.x * 0.78, y + 12)
			ul.size = Vector2(vp.x * 0.17, 28)
...
func _show_titles() -> void:
	...
	for t in MetaManager.TITLES:
		...
		need.position = Vector2(vp.x * 0.07, y + 24)
		need.size = Vector2(vp.x * 0.5, 18)
		...
		if unlocked and not is_active:
			...
			equip.text = "Equip"
			equip.position = Vector2(vp.x * 0.78, y + 7)
			equip.size = Vector2(vp.x * 0.17, 30)
			...
			active_l.position = Vector2(vp.x * 0.78, y + 10)
			active_l.size = Vector2(vp.x * 0.17, 24)
			active_l.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
			active_l.add_theme_font_size_override("font_size", 12)
